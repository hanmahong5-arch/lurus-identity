...
func (h *WebhookHandler) EpayNotify(c *gin.Context) {
	...
	if err := h.processOrderPaid(c, orderNo); err != nil {
...
func (h *WebhookHandler) processOrderPaid(c *gin.Context, orderNo string) error {
	order, err := h.wallets.MarkOrderPaid(c.Request.Context(), orderNo)
	if err != nil {
		return err
	}
	// If this order was for a subscription, activate it.
